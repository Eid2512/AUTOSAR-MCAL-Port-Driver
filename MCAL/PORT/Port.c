...
{
    #if (PORT_DEV_ERROR_DETECT == STD_ON)
    /* Check if the input configuration pointer is NULL */
    ...
    /* Mark the Port module as initialized */
    ...
    {
        volatile uint32* PortGpio_Ptr = NULL_PTR; /* Pointer to GPIO Port Registers */
        volatile uint32 delay = 0;

        /* Select the corresponding port base address */
        ...
        delay = SYSCTL_REGCGC2_REG; /* Allow time for clock to stabilize */

        /* JTAG Pins Handling (PC0-PC3) */
        if ((Port_ConfigPtr->Pin[index].port_num == 2) && (Port_ConfigPtr->Pin[index].pin_num <= 3))
        {
            /* *** WARNING: These pins are used for JTAG debugging. ***
             * Modifying them may disable the debugger and prevent further programming.
             * If debugging is lost, a full chip erase may be required.
             */
            continue; /* Skip configuration of JTAG pins */
        }

        /* Unlock and commit register for protected pins (PD7, PF0) */
        if (((Port_ConfigPtr->Pin[index].port_num == 3) && (Port_ConfigPtr->Pin[index].pin_num == 7)) ||
            ((Port_ConfigPtr->Pin[index].port_num == 5) && (Port_ConfigPtr->Pin[index].pin_num == 0)))
        {
            *(volatile uint32*)((volatile uint8*)PortGpio_Ptr + PORT_LOCK_REG_OFFSET) = 0x4C4F434B; /* Unlock GPIOCR */
